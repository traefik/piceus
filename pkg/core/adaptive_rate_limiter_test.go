...
func TestNewAdaptiveRateLimiter(t *testing.T) {
	...
	assert.Equal(t, 15, rl.safetyBuffer)
	...
	for _, tt := range tests {
		t.Run(tt.desc, func(t *testing.T) {
			...
			if tt.expectWaitTime {
				assert.True(t, waitTime > 0, "Expected positive wait time")
			} else {
...
func TestAdaptiveRateLimiter_ConcurrentAccess(t *testing.T) {
	...
	require.True(t, rl.remaining >= 0)
	require.True(t, rl.safetyBuffer > 0)
}
...
func TestAdaptiveRateLimiter_RealWorldScenario(t *testing.T) {
	...
	assert.True(t, waitTime <= 1*time.Minute) // Should be reasonable
}
